...
class MPPI():
    ...
    def __init__(self, f_cont, cost, const_cost, param, costN = None, use_filter=True):

        self.N = param.get("N", 40) # Horizon samples
        self.T = param.get("T", 4) # Time horizon
        self.K = param.get("K", 1000) # Number of discretization steps per timestep
        self.Sig = param.get("Sigma", [0.5, 0.025] ) # Covariance of sampling
        self.alpha = param.get("alpha",0.01) # exploration ratio
        self.lambd = param.get("lambda", 100)  # Temperature parameter

        self.gamma = 0.02*self.lambd
        self.Sigma = np.diag(self.Sig)
        self.dt = self.T/self.N
        self.invSigma = np.linalg.inv(self.Sigma)

        # Control limits
        self.umin = np.array([-3.0,-1.0])
        self.umax = np.array([ 3.0, 1.0])
    ...
    def solve(self, x0, r):
        ...
        # Elitism (keep best candidate from last time step)
        eps[:,:,0] = 0
